...
export function startProcess(type: string){
    ...
    Debug.log(`Started process ${type} with PID: ${process.pid}`, "SUCCESS", "Process");
}
...
export function stopProcess(type: string){
    ...
    global.DATA.TERMINAL_SESSIONS[type as keyof typeof global.DATA.TERMINAL_SESSIONS].STATUS = false
    Debug.log(`Stopped process ${global.DATA.TERMINAL_SESSIONS[type as keyof typeof global.DATA.TERMINAL_SESSIONS].PROCESS_PID}`, "SUCCESS", "Process");
}
